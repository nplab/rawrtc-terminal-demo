...
void data_channel_message_handler(
    ...
    if (flags & RAWRTC_DATA_CHANNEL_MESSAGE_FLAG_IS_BINARY) {
        ...
        int length = snprintf(mbuf_buf(buffer), mbuf_get_space(buffer), "{\"roll\": %d, \"pitch\": %d, \"yaw\": %d, \"valcount\": %d}\r\n", roll, pitch, yaw, calls++);

        mbuf_set_end(buffer, (size_t) length);
        rawrtc_data_channel_send(channel->channel, buffer, false);
        mem_deref(buffer);
    }
}
...
static void data_channel_handler(
    ...
    EOE(rawrtc_data_channel_set_buffered_amount_low_handler(channel, default_data_channel_buffered_amount_low_handler));
